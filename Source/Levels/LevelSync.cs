    ...
    public static class LevelSync
    {
        ...
        public static bool ReconcileRooftopsSlice(Map sky, ref int cursor, int maxCells)
        {
            ...
            try
            {
                ...
                for (; cursor < end; cursor++)
                {
                    ...
                    // Convergence for the mined-peak rule (existing saves and
                    // walls destroyed without a despawn event): natural roof
                    // with no rock wall under it on the TOPMOST level exposes
                    // open sky. Same future-levels caveat as the event path in
                    // OnLevelMineableDespawned.
                    if (topmostLevel)
                    {
                        RoofDef skyRoof = sky.roofGrid.RoofAt(c);
                        if (skyRoof != null && skyRoof.isNatural)
                        {
                            Building ed = sky.edificeGrid[c];
        ...
        public static void OnLevelMineableDespawned(Map levelMap, Thing thing)
        {
            ...
            try
            {
                ...
                // TOPMOST level only: mining exposes open sky. Nothing exists
                // above the top of the column, so a mined-out mass cell keeps
                // no overhead rock - carving into the peak yields open-air
                // notches (rooftop AA emplacements etc.). User directive after
                // run-34. NOTE: the gate is written as level == 1 because the
                // cap is three levels and the sky IS the peak; if additional
                // map levels are ever added, this must become "topmost level
                // of the column" or mined cells would wrongly expose sky with
                // another level overhead.
                if (levelMap.Level() == 1)
                {
                    RoofDef roof = levelMap.roofGrid.RoofAt(c);
                    if (roof != null && roof.isNatural)
                    {
                        levelMap.roofGrid.SetRoof(c, null);
                    }
                }
            }
            catch (Exception e)
        ...
        /// <summary>Maps a mining leave-terrain to the ROCK DEF whose mining produces
        /// it, so renderers can use the rock's own linked atlas graphic. Built in the
        /// same pass as the color map.</summary>
