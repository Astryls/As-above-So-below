...
namespace AsAboveSoBelow
{
    ...
    internal static class Patch_StartFormingCaravan_Column
    {
        ...
        private static readonly AccessTools.FieldRef<LordJob_FormAndSendCaravan, IntVec3> MeetingPointRef =
            AccessTools.FieldRefAccess<LordJob_FormAndSendCaravan, IntVec3>("meetingPoint");
        ...
        private static void Postfix(List<Pawn> pawns)
        {
            ...
            try
            {
                ...
                Map ground = lord.Map;
                // Route riders toward the caravan's meeting point when the lord
                // job carries one, so they surface at the gathering, not across
                // the map from it.
                IntVec3 meet = IntVec3.Invalid;
                if (lord.LordJob is LordJob_FormAndSendCaravan formJob)
                {
                    meet = MeetingPointRef(formJob);
                }
                int sent = 0;
                for (int i = 0; i < trimmed.Count; i++)
                {
                    Pawn p = trimmed[i];
                    if (!CrossLevelWork.TryStairsJobToward(p, ground, meet, out Job ride))
                    {
