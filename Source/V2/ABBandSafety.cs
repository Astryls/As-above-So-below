...
namespace AsAboveSoBelow
{
    ...
    public static class ABBandSafety
    {
        ...
        // ---- scatterer footprints -----------------------------------------

        /// <summary>Open-instance accessor for the ruin family's declared footprint. Pure
        /// type metadata, so unlike the statics §57f burned us on it describes nothing that
        /// can go stale between maps or between games.</summary>
        private static Func<GenStep_ScatterRuinsSimple, IntVec3, CellRect> ruinFootprint;

        /// <summary>Footprint accessors for scatterers OUTSIDE that family that follow the
        /// same convention anyway. Keyed by concrete type; a null value means "asked, does
        /// not have one", so an unusual genstep costs one reflection lookup per game rather
        /// than one per candidate cell.</summary>
        private static readonly Dictionary<Type, MethodInfo> foreignFootprints =
            new Dictionary<Type, MethodInfo>();

        private static bool footprintResolved;

        /// <summary>
        /// The rect a scatterer will actually BUILD at this cell, not the cell it was asked
        /// about. Falls back to the single cell when the genstep declares no footprint, which
        /// makes every caller degrade to the old point test rather than to nothing.
        ///
        /// <c>EffectiveRectAt</c> is resolved by NAME, on the concrete type, rather than by
        /// naming the classes that have one: it is the convention vanilla's ruin family uses,
        /// KCSG-style layout scatterers copy it, and resolving it this way means a mod's own
        /// scatterer is covered without this file ever hearing of it. Virtual dispatch does
        /// the rest - GenStep_ScatterShrines overrides the method to return its own random
        /// size, and the open delegate calls the override.
        /// </summary>
        internal static CellRect FootprintAt(GenStep_Scatterer step, IntVec3 loc)
        {
            CellRect single = CellRect.SingleCell(loc);
            if (step == null)
            {
                return single;
            }
            try
            {
                if (!footprintResolved)
                {
                    ...
                    ruinFootprint = BuildRuinFootprint();
                }
                CellRect r;
                if (step is GenStep_ScatterRuinsSimple ruins && ruinFootprint != null)
                {
                    r = ruinFootprint(ruins, loc);
                }
                else
                {
                    MethodInfo m = ForeignFootprint(step.GetType());
                    if (m == null)
                    {
                        return single;
                    }
                    r = (CellRect)m.Invoke(step, new object[] { loc });
                }
                // A generator that has not decided its size yet reports an empty rect. Taking
                // that literally would test a zero-height footprint and quietly re-introduce
                // the point test with extra steps.
                return r.Width > 0 && r.Height > 0 ? r : single;
            }
            catch
            {
                ruinFootprint = null;
        ...
        private static Func<GenStep_ScatterRuinsSimple, IntVec3, CellRect> BuildRuinFootprint()
        {
            try
            {
                MethodInfo m = AccessTools.Method(typeof(GenStep_ScatterRuinsSimple),
                    "EffectiveRectAt", new Type[] { typeof(IntVec3) });
                if (m == null)
                {
                    return null;
                }
                return (Func<GenStep_ScatterRuinsSimple, IntVec3, CellRect>)Delegate.CreateDelegate(
                    typeof(Func<GenStep_ScatterRuinsSimple, IntVec3, CellRect>), m);
            }
            catch
            {
                // Not fatal and not worth a warning: the point test still applies and the
                // rescue layer still catches whatever straddles.
                return null;
            }
        }

        private static MethodInfo ForeignFootprint(Type type)
        {
            if (foreignFootprints.TryGetValue(type, out MethodInfo cached))
            {
                return cached;
            }
            ...
            try
            {
                MethodInfo m = AccessTools.Method(type, "EffectiveRectAt",
                    new Type[] { typeof(IntVec3) });
                if (m != null && m.ReturnType == typeof(CellRect) && !m.IsStatic)
                {
                    found = m;
                }
            }
    ...
    public static class Patch_GenSpawn_ABNoVoidSpawn
    {
        private static void Prefix(Thing newThing, ref IntVec3 loc, Map map, bool respawningAfterLoad)
        {
            try
            {
                ...
                // ⚠ §57: stand down for the carve, for the same reason as the air guard.
                // ABCavernGen seeds cavern wildlife into the basement bands on purpose; a
                // live "move it to the surface" net would surface every cave animal.
    ...
    ///
    /// ⚠ IT IS THE FOOTPRINT THAT MUST CLEAR THE SEAM, NOT THE SPOT (rule 10: A STRUCTURE IS
    /// NOT A CELL). Vetting `loc` alone reads as sufficient because the margin is generous -
    /// 7 cells on a 190 band - right up until you notice the caller. GenStep_ScatterRuinsSimple
    /// draws its side from a 6..30 curve and CENTRES the ruin on the spot, so a 21-wide one
    /// sitting a comfortable 8 cells inside the band still hangs 2 cells into the level below
    /// and becomes a straddler. The margin was never the wrong number; it was being measured
    /// from the wrong thing, and no value of it could have fixed a rect that is wider than
    /// twice itself. Asking the genstep for its own EffectiveRectAt makes the test exact for
    /// every size it can roll.
    ///
    /// ⚠ AND THE MARGIN SHRINKS RATHER THAN SURRENDERS. The old "band too short, keep vanilla"
    /// escape has to go, because a big footprint can now exhaust the margin on a perfectly
    /// ordinary band, and dropping the constraint entirely at that moment would hand the seam
    /// straddlers back on exactly the maps most likely to produce them. The margin is a
    /// courtesy; FITTING INSIDE THE LEVEL IS THE REQUIREMENT. So the courtesy is trimmed until
    /// the requirement fits, which always leaves at least one legal row and therefore can
    /// never starve a scatterer into "could not find cell to generate at" - the failure mode
    /// that made SeamMarginFor derived rather than constant in the first place. A footprint
    /// taller than a whole level is refused a verdict here and left to the rescue layer, which
    /// can at least name it in the log.
    /// </summary>
    [HarmonyPatch(typeof(GenStep_Scatterer), "CanScatterAt")]
    public static class Patch_GenStep_Scatterer_ABBandSeam
    {
        private static void Postfix(GenStep_Scatterer __instance, IntVec3 loc, Map map,
            ref bool __result)
        {
            try
            {
                ...
                CellRect foot = ABBandSafety.FootprintAt(__instance, loc);
                int spare = band.Height - foot.Height;
                if (spare < 0)
                {
                    return; // cannot fit in a level at all; ABStructureFit will say so
                }
                ...
                if (foot.minZ < band.minZ + m || foot.maxZ > band.maxZ - m)
                {
                    __result = false;
                }
            }
