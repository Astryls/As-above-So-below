...
namespace AsAboveSoBelow
{
    ...
    public static class ABGenProfile
    {
        ...
        {
            // ⚠ PUBLISH HYGIENE. This is a dev instrument and it fires on EVERY banded map
            // generation, so shipping it ungated drops a 40-line YELLOW WARNING block into
            // every player's log on every new colony - noise for them, and a false lead for
            // any other author who opens that log to triage something unrelated. Behind
            // verboseLogging (Diagnostics tab) it is one click away, which is exactly how
            // §47.2's map-gen waste numbers were measured. Bail before the StringBuilder
            // work, not just before the log call: the census walks two def dictionaries.
