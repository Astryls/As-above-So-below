...
namespace AsAboveSoBelow
{
    ...
    /// <summary>
    /// THE AIM PIE - the fifth consumer of the raw target cell, arriving exactly as
    /// ABCombatV2's banner predicted ("there is no fifth symptom class; if a new one
    /// appears, it is a fifth consumer of the raw cell that has not been found yet").
    ///
    /// Stance_Warmup.StanceDraw hands GenDraw.DrawAimPie the live focusTarg, and the pie's
    /// facing comes from target.Thing.DrawPos - the cross-band target's REAL position, a
    /// whole Slot away. So the gun sprite (localized by the focusTarg swap above) points at
    /// the target as rendered while the warmup pie points through the stack, due north or
    /// south: "the aiming time indicator doesn't follow the weapon".
    ///
    /// Localizing the target cell makes vanilla take its cell branch
    /// ((cell - shooter.Position).AngleFlat), the honest on-screen bearing. Dropping
    /// DrawPos for Cell costs sub-cell precision, bounded by one cell and invisible in a
    /// pie arc. Runs every frame while a warming pawn is selected: two band reads and a
    /// translate, nothing allocated.
    /// </summary>
    [HarmonyPatch(typeof(GenDraw), nameof(GenDraw.DrawAimPie))]
    public static class Patch_GenDraw_ABCrossBandAimPie
    {
        private static void Prefix(Thing shooter, ref LocalTargetInfo target)
        {
            try
            {
                if (shooter == null || !target.IsValid)
                {
                    return;
                }
                if (ABCombatGeometry.TryLocalize(shooter, target.Cell, out IntVec3 local))
                {
                    target = new LocalTargetInfo(local);
                }
            }
            ...
            {
                // The pie is cosmetic; never let it break the draw loop.
            }
        }
    }
}
