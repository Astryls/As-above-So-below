...
namespace AsAboveSoBelow
{
            ...
            ref ShootLine resultingLine, ref bool __result, bool ignoreRange)
        {
            try
            {
                ...
                if (ABCombatV2.TrySolve(__instance, root, targ, out ABShotSolution sol,
                        ignoreRange))
                {
                    // ⚠ GATED AT THE CALLER (§66b). ABV2Debug.Combat checks the toggle, but
                    // the string CONCATENATION in its argument list runs first regardless -
                    // and this line sits on the stance recheck (every tick per aiming pawn)
                    // and the targeter (every frame). Ungated, it is pure GC garbage.
                ...
                {
                    // Same §66b gate as the OK branch: this one fires every recheck tick for
                    // as long as a cross-band pair stays unsolvable.
                    if (ABV2Debug.LogCombat)
                    {
    ...
    {
        /// <summary>Prefix-to-postfix carry. <see cref="band"/> is the destination's band
        /// (-1 = not banded, do nothing); <see cref="cross"/> records whether the shoot
        /// line SPANNED bands on entry - the discriminator for the §66c rebind, so a
        /// same-band wild miss can never re-key a concurrent cross-band shooter's parked
        /// solution.</summary>
        public struct MissState
        {
            public int band;
        ...
        private static void Prefix(ref ShootLine __instance, Map map, out MissState __state)
        {
            __state.band = -1;
            __state.cross = false;
            try
            {
                ...
                if (bands != null && bands.Banded)
                {
                    __state.band = bands.BandOf(__instance.Dest);
                    IntVec3 src = __instance.Source;
                    if (bands.BandOf(src) != __state.band)
                    {
                        __state.cross = true;
                        __instance = new ShootLine(bands.Translate(src, __state.band),
                            __instance.Dest);
                    }
                }
            }
        ...
        private static void Postfix(ref ShootLine __instance, Map map, MissState __state)
        {
            try
            {
                if (__state.band < 0)
                ...
                if (bands.BandOf(dest) != __state.band || bands.InGutter(dest))
                {
                    __instance = new ShootLine(__instance.Source,
                        ABCombatV2.ClampIntoBand(map, bands, dest, __state.band));
                }
                // §66c: the launch will be handed THIS dest, not the intended target cell
                // the solution was parked under. Re-key the pending record to the final
                // wild cell - AFTER the clamp, so the key is the cell Launch will actually
                // present. Cross-band lines only; the relay re-checks tick + band itself.
                if (__state.cross)
                {
                    ABCombatRelay.RebindPendingDest(__state.band, __instance.Dest);
                }
            }
            catch (Exception e)
            {
