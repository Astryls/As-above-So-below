...
namespace AsAboveSoBelow
{
    ...
    /// <summary>
    /// Left-click selection, band-aware in BOTH directions.
    ///
    /// ADD what you can see below: a pawn or building visible through open air is appended
    /// by translating the click down, mirroring the right-click rule exactly.
    ///
    /// REMOVE what you cannot see at all: the camera may overhang the band edge (panMargin,
    /// wide-zoom camera mods with clampZoomToLevel off), and the curtain past the edge is
    /// REAL MAP - the neighbouring band and the gutter, undrawn but fully populated. Vanilla
    /// happily selects there, which in play is "you can select things in the void" (window-7
    /// field report): an invisible stockpile, a colonist on the level below the seam. The
    /// rule that fixes it is the one the renderer already enforces: SELECTABLE IS DRAWABLE.
    /// A click whose cell is outside the viewed band's rows selects nothing, exactly as the
    /// clip draws nothing there; the bounds are taken from the same TryBandBounds the clip
    /// uses, so the two cannot drift apart.
    ///
    /// Two deliberate exemptions:
    ///   - The colonist bar. Vanilla yields its hit from a SCREEN-SPACE test and skips the
    ///     map path entirely; the pawn may legitimately be on any band. Touching that would
    ///     break portrait clicks for anyone not on the viewed level.
    ///   - Things ADDED by the see-below translation: their real cells are a band down BY
    ///     DESIGN. The in-band filter runs on vanilla's list, before the append.
    ///
    /// The in-band THING filter (not just the void wholesale-empty) exists for maps whose
    /// band height is an exact slot multiple: there the gutter is zero rows, the seam is one
    /// cell wide, and GenUI.ThingsUnderMouse's ~1-cell hit radius can straddle it - clicking
    /// the top row of your level could grab a pawn standing on the bottom row of the level
    /// above. With a non-zero gutter the radius cannot reach across; the filter is then a
    /// no-op that costs one BandOf per candidate, per click.
    /// </summary>
    ...
    public static class Patch_Selector_ABSelectThrough
    {
        ...
        private static void Postfix(ref System.Collections.Generic.IEnumerable<object> __result)
        {
            try
            {
                ...
                ABBandMap bands = ABBands.CompOf(map);
                // Same source and rounding as Patch_CameraDriver_ABClipViewToBand:
                // selectable and drawable must agree to the row.
                float bMin = 0f;
                float bMax = 0f;
                bool banded = bands != null && bands.Banded
                    && ABBandView.TryBandBounds(map, out bMin, out bMax);
                ...
                if (banded)
                {
                    // Screen-space colonist-bar hit: vanilla's map path never ran, the
                    // result is that one pawn/corpse, and band rules do not apply to UI.
                    if (Find.ColonistBar != null
                        && Find.ColonistBar.ColonistOrCorpseAt(UI.MousePositionOnUIInverted) != null)
                    {
                        return;
                    }
                    if (mouseCell.z < loZ || mouseCell.z > hiZ)
                    {
                        // The curtain void. Nothing is drawn here; nothing selects here.
                        __result = System.Linq.Enumerable.Empty<object>();
                        return;
                    }
                }

                // Materialised exactly once: the original is a lazy iterator whose body is
                // GenUI.ThingsUnderMouse plus zone/plan lookups, and enumerating it twice
                // would run all of that twice per click.
                ...
                {
                    originalCount++;
                    if (banded && o is Thing vt && vt.Spawned)
                    {
                        IntVec3 pos = vt.PositionHeld;
                        if (pos.z < loZ || pos.z > hiZ)
                        {
                            continue; // seam bleed from the hit radius on a gutterless map
                        }
                    }
                    extra.Add(o);
                }

                bool changed = extra.Count != originalCount;
                ...
                if (ABBelowClickThrough.TryTranslate(map, mouse, out Vector3 t))
                {
                    ...
                    if (belowCell.InBounds(map))
                    {
                        System.Collections.Generic.List<Thing> things =
                            map.thingGrid.ThingsListAtFast(belowCell);
