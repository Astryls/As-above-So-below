...
namespace AsAboveSoBelow
{
    ...
    public class SectionLayer_ABBelowShadows : SectionLayer_Dynamic
    {
            ...
            try
            {
                ...
                for (int x = rect.minX; x <= rect.maxX; x++)
                {
                    for (int z = rect.minZ; z <= rect.maxZ; z++)
                    {
                        ...
                        // ⚠ EVERY CANDIDATE DEPTH, SHALLOWEST FIRST - NOT ONE COMMITTED DEPTH.
                        //
                        // THE BUG THIS FIXES SHIPPED, and it is the one-descent family's
                        // ninth appearance wearing a new costume: the descent arithmetic was
                        // right, the CHOICE of descent was wrong. TryResolveDropAround used
                        // to return the MINIMUM drop over the 3x3 and this loop committed to
                        // it. From a sky band over open air the cell's own column resolves
                        // all the way to the ground, but any neighbour sitting over solid
                        // level-1 resolves to one Slot - the minimum won, we then looked for
                        // a caster one Slot down, found the OPEN AIR that made the column
                        // see-through in the first place, and skipped the cell entirely. So
                        // ground shadows vanished from level +2 exactly where level +1 had
                        // structures nearby, which is precisely where a colony is.
                        //
                        // Preferring the shallowest is still correct - a nearer caster
                        // occludes a deeper one - the error was deciding the depth BEFORE
                        // asking whether anything casts there. So: gather the candidates,
                        // walk them shallowest first, and take the first that actually holds
                        // a caster. A barren shallow candidate can no longer veto a real
                        // deep one, and the mountain-cap case still works because the
                        // neighbour-borrowed drops are still in the list.
                        TryResolveDropsAround(map, bands, here, dropCandidates);
                        IntVec3 below = IntVec3.Invalid;
                        Building b = null;
                        for (int ci = 0; ci < dropCandidates.Count; ci++)
                        {
                            IntVec3 cand = new IntVec3(x, 0, z - dropCandidates[ci]);
                            if (!cand.InBounds(map) || bands.InGutter(cand) || fog.IsFogged(cand))
                            {
                                continue;
                            }
                            Building cb = edifices[indices.CellToIndex(cand)];
                            if (cb == null || !(cb.def.staticSunShadowHeight > 0f))
                            {
                                continue;
                            }
                            below = cand;
        ...
        /// <summary>Scratch list for the candidate depths. One instance: section layers
        /// regenerate on the main thread and this runs per cell of a 17x17 section, so a
        /// fresh List per cell would be pure GC churn on every terrain change.</summary>
        private static readonly List<int> dropCandidates = new List<int>(9);

        /// <summary>
        /// EVERY depth this cell could be seeing a caster at, ordered shallowest first and
        /// deduplicated.
        ///
        /// The cell's OWN column is what it actually sees, so it is authoritative when it
        /// resolves; the eight neighbours are borrowed depths, and borrowing is what makes
        /// mountain shadows work at all (rock sits under an opaque cap, so its own column
        /// never resolves, while the shadow it throws lands on open ground beside it).
        ///
        /// ⚠ THIS RETURNS A LIST RATHER THAN A WINNER ON PURPOSE. Its previous form picked
        /// the minimum and handed back a single int, which meant the caller could not
        /// recover when that depth turned out to be empty - see the banner at the call site.
        /// Choosing among candidates requires knowing what is AT each one, and only the
        /// caller has the edifice grid in hand.
        /// </summary>
        private static void TryResolveDropsAround(Map map, ABBandMap bands,
            IntVec3 c, List<int> into)
        {
            into.Clear();
            for (int i = 0; i < 9; i++)
            {
            ...
            // Shallowest first: a nearer caster occludes a deeper one, so the first candidate
            // that actually holds a caster is the one genuinely visible from up here.
            into.Sort();
        }
