...
namespace AsAboveSoBelow
{
    ...
    internal static class ABCameraBounds
    {
        ...
        /// <summary>
        /// ⚠ THE ZOOM TARGET, AND THE FIELD THAT MADE THE CLAMP FLICKER.
        ///
        /// Clamping `rootSize` alone clamps the CURRENT zoom and leaves the TARGET alone,
        /// and CameraDriver.Update walks the one towards the other every frame. So with a
        /// mod that widens `config.sizeRange` (Simple Camera Setting raises the max; other
        /// camera mods overwrite the whole range) the player parks `desiredSize` far past
        /// our cap and vanilla immediately re-closes the gap: without smooth zoom the step
        /// is `(desired - root) * 0.4`, so a 200-vs-63 gap renders at ~118 and is slammed
        /// back to 63 on the very next line. That oscillation IS the reported flicker and
        /// blur at full zoom-out - the camera really was rendering a different size every
        /// frame. Clamping the target as well removes the gap, so there is nothing left to
        /// oscillate.
        /// </summary>
        internal static readonly AccessTools.FieldRef<CameraDriver, float> DesiredSize =
            Resolve<float>("desiredSize");

        /// <summary>
        /// ⚠ A POSTFIX ON Update RUNS AFTER THE CAMERA HAS ALREADY BEEN POSITIONED.
        ///
        /// `Update` ends with `ApplyPositionToGameObject()`, which is what actually writes
        /// `MyCamera.orthographicSize` and `MyCamera.transform.position`. Anything a postfix
        /// does to `rootPos` / `rootSize` therefore does NOT reach the frame being rendered;
        /// it only seeds the next one. That is survivable on its own (one frame late) but it
        /// loses outright against any other mod that writes the camera transform directly in
        /// its own Update postfix - Perspective Shift does exactly that when it locks the
        /// view to its avatar. Re-applying after we clamp puts our value on the object the
        /// renderer reads, this frame, whoever ran before us.
        /// </summary>
        internal static void ApplyToGameObject(CameraDriver cam)
        {
            if (cam == null || applyPosition == null)
            {
                return;
            }
            try
            ...
            {
                // Cosmetic. A missed re-apply costs one frame of lag, never correctness.
            }
        }

        private static readonly Action<CameraDriver> applyPosition = ResolveApply();

        /// <summary>Field access that returns null instead of throwing in a type
        /// initialiser. A missing private field must degrade to "skip that clamp", not to a
        /// TypeInitializationException that takes the whole camera system with it.</summary>
        private static AccessTools.FieldRef<CameraDriver, T> Resolve<T>(string field)
        {
            try
            {
                return AccessTools.FieldRefAccess<CameraDriver, T>(field);
            }
            catch (Exception e)
            {
                Log.WarningOnce(ABLog.Tag + " V2: CameraDriver." + field + " not found ("
                    + e.Message + "); that part of the band camera clamp is inactive.",
                    0x2B10A7);
                return null;
            }
        }

        private static Action<CameraDriver> ResolveApply()
        {
            try
            {
                MethodInfo m = AccessTools.Method(typeof(CameraDriver),
                    "ApplyPositionToGameObject");
                return m == null
                    ? null
                    : AccessTools.MethodDelegate<Action<CameraDriver>>(m);
            }
            catch (Exception e)
            {
                Log.WarningOnce(ABLog.Tag + " V2: CameraDriver.ApplyPositionToGameObject not"
                    + " found (" + e.Message + "); band camera clamps land one frame late.",
                    0x2B10A8);
                return null;
            }
        }
