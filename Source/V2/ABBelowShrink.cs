...
namespace AsAboveSoBelow
{
    ...
    public static class Patch_PawnRenderer_ABBelowShrink
    {
        private static void Postfix(ref PawnDrawParms __result)
        {
            try
            {
                ...
                if (stair < 0.999f || Mathf.Abs(shimmy) >= 0.0005f)
                {
                    // The stair HALF specifically reached a matrix this frame - the depth
    ...
    /// silently evaporated at exactly the zoom levels a player uses to look down a column -
    /// colonists two levels down shrank while zoomed in and snapped back to full size when
    /// zoomed out, which reads as a flicker bug rather than a missing patch.
    ///
    ...
    /// pawn - vetoing only the cases the blit genuinely cannot express (gear, stair
    /// animation, unaffordable first-touch bakes) and only when it would actually be
    /// visible. It arms SuppressCache for exactly those.
    /// </summary>
