...
namespace AsAboveSoBelow
{
    ...
    public static class Patch_GenClosest_ABSameIslandFirst
    {
            ...
            bool lookInHaulSources)
        {
            ...
            try
            {
                // §59: the scanning pawn selects the partition, exactly as movement does.
                // Resolved ONCE here and carried into the per-candidate predicate - it is the
                // same one-bit question, and asking it per candidate would put
                // CaresAboutForbidden inside the scan loop. A null pawn (plenty of callers
                // pass one) resolves to the optimistic partition, i.e. today's behaviour.
                bool forbidAware = ABBandComponents.RespectsForbiddenDoors(traverseParams.pawn);
                if (map == null || !ABBands.Banded(map)
                    || !ABBandComponents.FragmentedBandAt(map, root, forbidAware))
                {
                    return true; // the common case: one bool per query on a settled band
                }
                int rootIsland = ABBandComponents.ComponentOf(map, root, forbidAware);
                if (rootIsland < 0)
                {
                    return true;
                }
                ...
                {
                    if (!IslandAllows(map, root, rootIsland, forbidAware, t))
                    {
                        return false;
                    }
        ...
        private static bool IslandAllows(Map map, IntVec3 root, int rootIsland,
            bool forbidAware, Thing t)
        {
            if (t == null)
            {
                return true;
            ...
            int isle = ABBandComponents.ComponentOf(map, pos, forbidAware);
