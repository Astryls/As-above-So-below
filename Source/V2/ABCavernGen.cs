...
namespace AsAboveSoBelow
{
    ...
    public static class ABCavernGen
    {
        ...
        public static void Generate(Map map, ABBandMap bands, int band)
        {
            ...
            BiomeDef biome = BiomesCavernsCompat.Resolve(settings?.basementBiomeChoice
        ...
        private static void Carve(Map map, ABBandMap bands, int band, BiomeDef biome, float openness)
        {
            ...
            ABLog.Dev("Cavern basement (band " + band + "): " + biome.defName
            ...
            // commonality and gated by terrain fertility. Regrowth afterwards is vanilla
            // WildPlantSpawner business, which now follows the band biome on its own.
            List<ThingDef> plants = biome.AllWildPlants?.ToList() ?? new List<ThingDef>();
            if (plants.Count > 0)
            {
